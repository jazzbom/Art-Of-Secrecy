...
public class Steganography
{
	 ...
	 */
	public InputStream encode(InputStream img, String ext1, String message) throws IOException
	{
		InputStream is = null;
		//String			file_name 	= image_path(path,original,ext1);
		BufferedImage 	image_orig	= ImageIO.read(img);
		
		//user space is not necessary for Encrypting
		BufferedImage image = user_space(image_orig);
		image = add_text(image,message);
		
		System.out.println("img path---"+image.getData().toString());
	 ...
	 *@param type integer representing either basic or advanced encoding
	 */
	public String decode(InputStream img)
	{
		byte[] decode;
		try
		{
        ...
        catch(Exception e)
        {
			...
			return "There is no hidden message in this image!";
        }
    }
	...
	private BufferedImage add_text(BufferedImage image, String text)
	{
		...
		{
			System.out.println("Target File cannot hold message! Error");
		}
		return image;
	}
